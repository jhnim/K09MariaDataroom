...
public class WriteCtrl extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
		 글쓰기 페이지로 진입할때는 다른 처리없이 포워드만 하면된다.ㄴ
		 */
		req.getRequestDispatcher("/14Dataroom/DataWrite.jsp").forward(req, resp);
	}
	
	/*
	 글작성을 완료한 후 서버로 전송(submit)할때의 요청을 처리한다. 게시판에 글을 쓸때는 post방식으로 처리하게 된다. 
	 */
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(mr != null) {
			/*
			 파일업로드가 완료되면 나멈지 폼값을 받기위해 mr참조변수를 이용한다.
			 */
