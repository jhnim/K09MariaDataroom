...
<script>
	function checkValidate(fm){
		...
		if(fm.content.value==""){
            ...
            <!--  
            	파일업로드를 위해서는 반드시 enctype을 선언해야한다.
            ...
            <!--  
            	기존에 등록한 파일이 있는 경우 수정시 파일을 첨부하지 않으면 기존파일을 유지해야 하므로 별도의 hidden폼이 필요하다.
            	즉, 새로운 파일을 등록하면 새로운 값으로 업데이트하고, 파일을 등록하지 않으면 기존 파일명으로 데이터를 유지하게 된다.
            -->
            <colgroup>
               <col width="20%"/>
               <col width="*"/>
         ...
         <div class="row mb-3">
            <div class="col text-right">
               ...
               <button type="button" class="btn btn-warning" onclick="location.href='../DataRoom/DataList?nowPage=${param.nowPage}';">리스트바로가기</button>
            </div>
            </form>
         </div>
            <!-- ### 게시판의 body 부분 end ### -->
